...
- **Article dateline is viewer-local, IST is the fallback.** The article kicker renders `<time datetime="<UTC ISO>">` with server text from `publisher._format_datetime()` (UTC→IST). `web/static/js/localtime.js` (deferred, CSP-`self`) rewrites the visible text to the *viewer's* timezone via `Intl.DateTimeFormat`. So the displayed time is the visitor's local time, NOT IST and NOT the DB value directly — don't be surprised it differs per viewer. If JS is off or `Intl` is missing, the server IST string stays. The underlying `Article.published_at` is set at first publish and not reprocessed afterward (published clusters aren't re-run — see the "Published clusters are never reprocessed" note below), same field the homepage `reltime` uses. Re-render existing pages with `scripts/rerender_articles.py` after touching the dateline.
...
- **Published clusters are never reprocessed.** `publish_top_n` only selects `is_published == False` clusters and nothing in the codebase ever resets `is_published` back to False. So a published article is written **once** — it does NOT re-run the writer or re-pick its image on a cycle. (Earlier notes here claiming `published_at` is "overwritten on every ~2h republish" were inaccurate: `published_at` is effectively set at first publish and left.) Consequence: changes to the writer/imager only affect **new** articles automatically; fixing existing ones requires an explicit script (`rewrite_articles.py` for bodies, `repick_images.py` for images).
