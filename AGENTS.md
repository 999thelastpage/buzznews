...
- **LLM writer chain**: DeepSeek (primary) → Gemini (capped fallback) → Anthropic Haiku (2nd fallback). DeepSeek uses OpenAI-compat at `https://api.deepseek.com/v1/chat/completions` via `httpx` — model ids are **lowercase** (`deepseek-v4-flash`, `deepseek-v4-pro`). Gemini fallback is capped by `GEMINI_FALLBACK_DAILY_CAP` and recorded in `llm_usage_events`.
