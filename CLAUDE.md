...
- **DB backups**: nightly `pg_dump` of the Neon DB at 21:30 UTC (03:00 IST) via `buzz-news-backup.timer`. Dumps land in `/var/backups/buzz-news/` (last 3 kept locally) and are rsync'd over SSH to a separate VPS that runs `remote-prune.sh` for 7-daily/4-weekly/12-monthly retention. Health check: `systemctl list-timers buzz-news-backup` (next-fire shown, no `failed`); last run log at `/var/log/buzz-news/backup.log`. Do NOT re-add backups as an APScheduler job — ops layer stays out of the app process. SSH key at `/home/ubuntu/.ssh/buzz_backup_ed25519` is automation-only (`from=`/`command=` locked in the remote `authorized_keys`).
